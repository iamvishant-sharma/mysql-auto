    ...
    stages {
       stage ('automating...') {
            steps {
                ...
                sh "ssh kubecli@47.91.111.100 -p ${port} ansible-playbook mysql-auto/site.yaml -e 'login_user=root login_password=7fpJ3ffm9AHYPOe563U9 db_name=${db_name} username=${username} password=${password} priv=s${privileges}'"
            }
        }
    }
}
